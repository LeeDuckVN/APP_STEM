    ...
    .map((category) => {
      ...
      return `
        <button class="mega-item ${activeCategory === category.id ? "active" : ""}" type="button" data-mega="${category.id}">
          <span>${category.icon} ${label(category.label)}</span>
          <strong>${count}</strong>
          <small>${preview}</small>
        </button>
      `;
...
function getVisibleProducts() {
  ...
  let visible = products.filter((product) => {
    ...
    return categoryOk && subOk && scopeOk && stockOk && matchesPrice(product) && searchOk;
  });
          ...
          <div class="product-body">
            ...
            <div class="card-actions">
              <div class="qty-control">
                <button class="qty-button" type="button" data-step="-1" data-qty-for="${product.id}">−</button>
                <input id="qty-${product.id}" type="number" value="1" min="1" max="${product.inventory}" aria-label="${t("quantityFor", { name: productName })}" />
                <button class="qty-button" type="button" data-step="1" data-qty-for="${product.id}">+</button>
              </div>
              <button class="buy-button" type="button" data-buy="${product.id}">${t("add")}</button>
            </div>
          </div>
        </article>
      `;
    ...
    .map(
      (product) => `
        <article class="mini-card">
          <div class="product-image" style="--image-position: ${product.imagePosition}">
            <img src="${product.image}" alt="${label(product.name)}" loading="lazy" />
          </div>
          <div>
...
function renderSuggestions() {
  ...
  const matches = products.filter((product) => productSearchText(product).includes(query)).slice(0, 5);
          ...
          (item) => `
            <div class="cart-line">
              ...
              <div class="cart-line-actions">
                <button type="button" data-cart-step="-1" data-cart-id="${item.id}">−</button>
